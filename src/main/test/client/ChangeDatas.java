...
public class ChangeDatas {
	...
	public void checkReadData(String msg)throws Exception{
		...
		String lastSenStr = new String(lastSen, "euc-kr").trim(); // 정상 처리되었습니다.

		if(lastSenStr.equals(successSentence)) {
			printData(msg);
		}else {
	...
	private void printData(String msg) throws Exception {
		...
		System.out.println("{길이부} = " + length); // 정상 처리 메세지 + 원래 길이부 길이
